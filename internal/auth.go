...
func ValidateCookie(r *http.Request, c *http.Cookie) (string, string, string, error) {
	...
	expectedSignature := cookieSignature(r, parts[4], parts[1])
...
func MakeCookie(r *http.Request, user provider.User) *http.Cookie {
	...
	mac := cookieSignature(r, user.ID, fmt.Sprintf("%d", expires.Unix()))
	value := fmt.Sprintf("%s|%d|%s|%s|%s", mac, expires.Unix(), user.Email, user.Name, user.ID)
